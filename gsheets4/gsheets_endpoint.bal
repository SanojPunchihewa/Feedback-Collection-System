...
public type Client client object {
    ...
    # + spreadsheetId - Id of the spreadsheet
    # + sheetName - Name of the sheet
    # + values - Values to be apended
    # + return - True on success and error on failure
    public remote function appendRow(string spreadsheetId, string sheetName, string[] values) returns (boolean)|error;
};
...
public remote function Client.appendRow(string spreadsheetId, string sheetName,
                                                  string[] values) returns boolean|error {
    http:Request request = new;
    json jsonPayload = {"values":[values]};
    string a1Notation = sheetName;
    string setCellDataPath = SPREADSHEET_PATH + PATH_SEPARATOR + spreadsheetId + VALUES_PATH + a1Notation
         + ":append" + QUESTION_MARK + INSERT_DATA_OPTION + AMPERSAND + VALUE_INPUT_OPTION;
   
    request.setJsonPayload(jsonPayload);
    var httpResponse = self.spreadsheetClient->post(setCellDataPath, request);
    if (httpResponse is http:Response) {
        int statusCode = httpResponse.statusCode;
        var jsonResponse = httpResponse.getJsonPayload();
        if (jsonResponse is json) {
